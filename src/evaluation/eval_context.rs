...
pub struct EvalContext {
    ...
    pub duplicates: HashMap<(String, BTreeMap<String, Option<String>>), i32>,
    ...
    pub cache: HashMap<(String, BTreeMap<String, Option<String>>), (GraphColoredVertices, HashMap<String, String>)>,
    ...
    pub free_var_domains: BTreeMap<String, Option<String>>,
}

impl EvalContext {
    /// Instantiate the struct with precomputed duplicates. Cache and domain-related fields are left empty.
    pub fn new(duplicates: HashMap<(String, BTreeMap<String, Option<String>>), i32>) -> EvalContext {
        EvalContext {
            duplicates,
            cache: HashMap::new(),
            domain_raw_sets: HashMap::new(),
            free_var_domains: BTreeMap::new(),
        }
    }
    ...
    pub fn get_duplicates(&self) -> &HashMap<(String, BTreeMap<String, Option<String>>), i32> {
        &self.duplicates
    }

    /// Get a ref to the `cache` field containing the cached sub-formulae, their result and var renaming.
    pub fn get_cache(&self) -> &HashMap<(String, BTreeMap<String, Option<String>>), (GraphColoredVertices, HashMap<String, String>)> {
        &self.cache
    }
    ...
    /// Get a ref to the `free_var_domains` field containing the cached domain names and the raw sets.
    pub fn get_free_var_domains(&self) -> &BTreeMap<String, Option<String>> {
    ...
    pub fn extend_context_with_wild_cards(
        ...
        for (prop_name, raw_set) in subst_context_properties.iter() {
            ...
            let sub_formula_with_domains = (sub_formula.clone(), BTreeMap::new());
            ...
            self.cache
                .insert((sub_formula, BTreeMap::new()), (raw_set.clone(), HashMap::new()));
        }
...
mod tests {
    ...
    fn generating_eval_context() {
        ...
        let expected_duplicates = HashMap::from([
            (("(AX {var0})".to_string(), BTreeMap::from([("var0".to_string(), None)])), 1)
        ]);
    ...
    fn eval_context_extending() {
        ...
        let expected_cache = HashMap::from([(("%subst%".to_string(), BTreeMap::new()), (raw_set_1, HashMap::new()))]);
        ...
        let expected_duplicates = HashMap::from([
            (("%subst%".to_string(), BTreeMap::new()), 2)
        ]);
